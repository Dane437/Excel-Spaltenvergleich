...
def highlight_differences(file_name, sheet_name, col_left, col_right, header_row, valid_combinations):
    ...
    for row in range(header_row + 1, ws.max_row + 1):
        ...
        if c1.value != c2.value:
            if valid_combinations is not None and (c1.value, c2.value) in valid_combinations:
                    continue
            else:
...
def create_differences_sheet(file_name, compare_column, sheet_title, rows_different, header_row):
    wb = load_workbook(file_name)
    ws = wb['Temp']
    headers = {cell.value: cell.column for cell in ws[header_row]}
    worksheet_copy = wb.copy_worksheet(ws)
    worksheet_copy.title = sheet_title
